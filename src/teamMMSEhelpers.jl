...
function generateTeamMMSEsamples(p::QuadTeamProblem, t::teamMMSEproblem, m::Int)
    ...
    H = [[sampleComplexNormal(t.μₕ[i], t.σₕ[i], p.N) for _ = 1:m] for i = 1:p.N]
    N = [[sampleComplexNormal(t.μₙ[i], t.σₙ[i], p.N) for _ = 1:m] for i = 1:p.N]

    Y = Vector{Vector}(undef, p.N)

    for i = 1:p.N
        A = [l == i ? H[l] : (1 - t.ϵ[i]) * H[l] .+ t.ϵ[i] * N[l] for l = 1:p.N]
    ...
    for i = 1:p.N
        for j = 1:p.N
            R[i, j] =
                i == j ? (H[i] .|> h -> h') .* H[j] .+ t.σᵣ : (H[i] .|> h -> h') .* H[j]
        end
    end
